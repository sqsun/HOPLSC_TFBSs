...
maxiters = params.Results.maxiters;% 50
dimorder = params.Results.dimorder;% 1 2 3
init = params.Results.init;% nvecs
printitn = params.Results.printitn;% 1
...
%                     R(n),n);
            Uinit{n} = nvecs(X,n,R(n));% 按照模式-n，计算奇异值的个数为R(n)
        end
    else
...
for iter = 1:maxiters
    ...
    for n = dimorder(1:end)
        Utilde = ttm(X, U, -n, 't');% 除去mode-n外,相应的做张量积
        
        % Maximize norm(Utilde x_n W') wrt W and
        % keeping orthonormality of W
        U{n} = nvecs(Utilde,n,R(n));%对上式做好的张量积进行奇异值分解
    end
