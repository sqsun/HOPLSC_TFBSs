function [T,Uinit] = tucker_als_new(X,R,varargin)
...
% Added by Qibin
if iscell(init) && any(R==-1)
    Uinit = cell(N,1);
        for n = dimorder(1:end)
            if R(n)~=-1 && isempty(init{n})
                fprintf('  Computing %d leadinghhh e-vectors for factor %d.\n', ...
                    R(n),n);
...
% calculate U, consider the approximation
for iter = 1:maxiters
    ...
    % Compute fit
    normresidual = sqrt( abs(normX^2 - norm(core)^2) );
    fit = 1 - (normresidual / normX); %fraction explained by model
    fitchange = abs(fitold - fit);

    if mod(iter,printitn)==0
        fprintf(' Iteration %2d: fit = %e fitdelta = %7.1e\n', iter, fit, fitchange);
    end
